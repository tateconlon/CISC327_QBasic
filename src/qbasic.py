...
class QBasic():
	...
	def run(self):
		...
		self.loggedIn = False
		#print('Welcome to QBasic!') [interactive]
		
		#log in
		while not self.loggedIn:
			#while(input('Please type login to log in: ') != "login"): [interactive]
			while(input() != "login"):
				pass
	...
	def startLoggedInState(self, permissionType):
		...
		if not self.loggedIn:
			print("You are not logged in, you cannot enter the logged in state!") #should never display
			return

		while (self.loggedIn):
			#transactionInput = input("Input the transaction code: ") [interactive]
	...
	def login(self):
		...
		#permissionType = input('Please enter desired permission type (agent/machine): ') [interactive]
		permissionType = input()
		if(permissionType not in ['agent', 'machine']):
			#print("login permission {0} invalid. Please choose agent or machine.".format(permissionType)) [interactive]
			print("login permission {0} not agent or machine".format(permissionType))
			return ""
	...
	def create_acct(self, permissionType):
		...
		#accountNumber = input('Please enter the account number (7 digits): ') [interactive]
		accountNumber = input()
		...
		if not self.isNameValid(name):
			#print('{0} is not a valid account name. Valid account names are 3-30 alpha characters long, no leading/trailing spaces, but spaces are allowed inside'.format(name)) [interactive]
	...
	def delete_acct(self, permissionType):
		...
		if not self.isNameValid(accountName):
			#print('Cannot delete account with name {0}. Valid account names are 3-30 alpha characters long, no leading/trailing spaces, but spaces are allowed inside'.format(name)) [interactive]
			print('{0} is not a valid account name'.format(name))
			return
	...
	def withdraw(self, permissionType):
		...
		#accountNumber = input('Please enter the account number to withdraw from: ') [interactive]
		accountNumber = input('')
		if(accountNumber not in self.validAccounts):
			print('Account {0} does not exist'.format(accountNumber))
			return

		#wdrAmtStr = input('Please enter the amount to withdraw (in cents): ') [interactive]
		wdrAmtStr = input('')
		try:
	...
	def deposit(self, permissionType):
		...
		if permissionType not in ['agent', 'machine']:
 		...
 		# Enter account number and check that it's valid
		#accountNumber = input('Please enter the account number to deposit into: ') [interactive]
		accountNumber = input('')
		if(accountNumber not in self.validAccounts):
			print('Account {0} does not exist'.format(accountNumber))
			return
		...
		#depAmtStr = input('Please enter the amount to deposit (in cents): ') [interactive]
		depAmtStr = input('')
		try:
	...
	def transfer(self, permissionType):
		...
		#accountNumber1 = input('Please enter the account number to tranfer from: ') [interactive]
		accountNumber1 = input('')
		if(accountNumber1 not in self.validAccounts):
			print('Account {0} does not exist'.format(accountNumber1))
			return

		#accountNumber2 = input('Please enter the account number to tranfer from: ') [interactive]
		accountNumber2 = input('')
